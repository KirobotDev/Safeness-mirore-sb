...
const TextSortableGroupTypes = ['GUILD_TEXT', 'GUILD_ANNOUNCEMENT', 'GUILD_FORUM'];
...
class Util extends null {
  ...
  static flatten(obj, ...props) {
    if (!isObject(obj) && typeof obj !== 'bigint') return obj;
    ...
    return Object.fromEntries(
      Object.entries(props)
        ...
        .map(([prop, newProp]) => {
          ...
          let value;
          if (element instanceof Collection) value = Array.from(element.keys());
          else if (valueOf instanceof Collection) value = Array.from(valueOf.keys());
          else if (Array.isArray(element)) value = element.map(e => (typeof e === 'bigint' ? e.toString() : Util.flatten(e)));
          else if (typeof element === 'bigint') value = element.toString();
          else if (typeof valueOf !== 'object') value = valueOf;
          else if (hasToJSON) value = element.toJSON();
          else if (typeof element === 'object') value = Util.flatten(element);
          else if (!elemIsObj) value = element;
      ...
      fetch(url, {
        ...
        .then(res => {
          if (res.ok) {
            ...
            reject(new HTTPError(res.statusText, res.constructor.name, res.status, { method: 'PUT', path: url }));
          }
        })
        .catch(reject);
    });
  ...
  /**
   * Duck-typed check for proxy agent objects.
   * Supports https-proxy-agent v4/v7, socks-proxy-agent, and any object
   * with both an httpAgent and httpsAgent property.
   * @param {*} object
   * @returns {boolean}
   */
  static verifyProxyAgent(object) {
    if (!object || typeof object !== 'object') return false;
    // Standard proxy-agent wrapper shape (https-proxy-agent v5+, socks-proxy-agent)
    if ('httpAgent' in object && 'httpsAgent' in object) return true;
    // Single-agent shape (some proxy packages expose one agent directly)
    const name = object?.constructor?.name ?? '';
    return name.includes('Agent') && typeof object.addRequest === 'function';
    // Agent shape (node:http-agent)
  }
