...
const TutorialFlags = require('../util/TutorialFlags');
...
class ClientUser extends User {
  ...
  _patch(data) {
    ...
    if ('tutorial_flags' in data) {
      /**
       * Tutorial flags of the client user.
       * @type {Readonly<TutorialFlags>}
       */
      this.tutorialFlags = new TutorialFlags(data.tutorial_flags || 0).freeze();
    } else {
      this.tutorialFlags ??= new TutorialFlags().freeze();
    }
  }
  ...
  /**
   * Acknowledge a tutorial flag to clear "NEW" badges.
   * @param {TutorialFlagsResolvable} flag 
   * @returns {Promise<ClientUser>}
   */
  async ackTutorial(flag) {
    const tutorialFlags = new TutorialFlags(this.tutorialFlags.bitfield);
    tutorialFlags.add(flag);
    return this.edit({ tutorial_flags: tutorialFlags.bitfield });
  }

  /**
   * Whether the user has Nitro.
   * @type {boolean}
   * @readonly
   */
  get isNitro() {
  ...
  /**
   * Whether the user can use external emojis.
   * @type {boolean}
   * @readonly
   */
  get canUseExternalEmojis() {
    return this.premiumType >= 1; // Nitro Basic has limited features, but can use external emojis
  }
  ...
  get maxUploadSize() {
    switch (this.premiumType) {
      case 1:
        return 52_428_800; // 50MB (Basic)
      case 2:
        return 524_288_000; // 500MB (Nitro)
      default:
        return 26_214_400; // 25MB (None)
    }
  }

  /**
   * Fetch the full profile of the client user.
   * @returns {Promise<ClientUser>}
   */
  async fetchProfile() {
