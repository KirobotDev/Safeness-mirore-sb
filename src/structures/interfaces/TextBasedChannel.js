...
/***
 * @example
 * Read TextBasedChannel For Comprhensions Basedchannel
 */
...
class TextBasedChannel {
   ...
   * @property {boolean | number} [typing=false] Whether to simulate typing before sending (true for auto-delay, or number for ms)
   */
  ...
  async send(options) {
    ...
    if (options.typing) {
      const waitTime = typeof options.typing === 'number' ? options.typing : (messagePayload.data.content?.length || 0) * 40 + 600;
  ...
  /**
   * Search for interactions/slash commands in the current guild or DM channel.
  ...
  /**
   * Search for interactions/slash commands installed on the user.
   * @returns {Promise<Object>}
   */
  ...
  /**
   * Search for interactions/slash commands.
   * @returns {Promise<Object>}
   */
  searchInteraction() {
  ...
  /**
   * Sends a slash command interaction.
   * @param {Snowflake} botOrApplicationId The bot or application ID to target
   * @param {string} commandNameString The command name (e.g. "poll create")
   * @param {...*} args Arguments for the command
   * @returns {Promise<Object>} The interaction response
   */
