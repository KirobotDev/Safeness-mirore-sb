...
class ClientUserSettingManager extends BaseManager {
  ...
  setCustomStatus(options) {
    if (typeof options !== 'object') {
      ...
      if (options.emoji) {
        ...
        } else if (typeof options.emoji === 'object' && options.emoji.id && options.emoji.name) {
          data.emoji_name = options.emoji.name;
          data.emoji_id = options.emoji.id;
          if (options.emoji.animated) data.emoji_animated = true;
        } else {
   ...
   * @param {number} level Filter level (0: Off, 1: Friends excluded, 2: Scan everyone)
   * @returns {Promise<this>}
   */
  setExplicitContentFilter(level) {
    if (![0, 1, 2].includes(level)) throw new TypeError('INVALID_VALUE', 'level', '0 | 1 | 2');
  ...
  setDeveloperMode(enabled) {
    if (typeof enabled !== 'boolean') throw new TypeError('INVALID_TYPE', 'enabled', 'boolean');
    return this.edit({ developer_mode: enabled });
  }

  /**
   * Set AFK timeout.
   * @param {number} seconds AFK timeout in seconds
   * @returns {Promise<this>}
   */
  setAfkTimeout(seconds) {
    if (typeof seconds !== 'number') throw new TypeError('INVALID_TYPE', 'seconds', 'number');
    return this.edit({ afk_timeout: seconds });
  }

  /**
   * Set friend source flags (Who can add you as a friend).
   * @param {Object} flags Source flags
   * @param {boolean} [flags.all] Everyone
   * @param {boolean} [flags.mutual_friends] Mutual friends
   * @param {boolean} [flags.mutual_guilds] Mutual guilds
   * @returns {Promise<this>}
   */
  setFriendSourceFlags(flags) {
    if (typeof flags !== 'object') throw new TypeError('INVALID_TYPE', 'flags', 'object');
    return this.edit({ friend_source_flags: flags });
  }

  /**
   * Set privacy settings.
   * @param {Object} settings Privacy settings
   * @param {boolean} [settings.allow_activity_party_privacy_friends] Allow friends to join your party
   * @param {boolean} [settings.allow_activity_party_privacy_everyone] Allow everyone to join your party
   * @param {boolean} [settings.default_guilds_restricted] Restricted guilds by default
   * @returns {Promise<this>}
   */
