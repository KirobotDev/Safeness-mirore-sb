...
class GuildManager extends CachedManager {
  ...
  async create(
    ...
    const data = await this.client.api.guilds.post({
      data: {
        ...
        guild_template_code: guildTemplateCode ?? '2TffvPucqHkN',
      },
    });
   ...
   * @param {GuildResolvable} [id] The guild to fetch
   * @param {BaseFetchOptions} [options] Additional options for this fetch
   * @returns {Promise<Guild|Collection<Snowflake, Guild>>}
   */
  async fetch(id, options) {
    if (!id) return this._fetchMany(options);

    const guildId = this.resolveId(id);
    if (!guildId) throw new Error('GUILD_INVALID');
    ...
    const data = await this.client.api.guilds(guildId).get({ query: { with_counts: options?.withCounts ?? true } });
    return this._add(data, options?.cache);
  }

  /**
   * Obtains multiple guilds from Discord.
   * @param {FetchGuildsOptions} [options] Options for fetching guilds
   * @returns {Promise<Collection<Snowflake, OAuth2Guild>>}
   * @private
   */
