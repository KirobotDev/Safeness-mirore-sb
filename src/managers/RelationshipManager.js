...
class RelationshipManager extends BaseManager {
  ...
  /**
   * Internal handler for new relationships.
   * @param {Object} data Relationship data
   * @private
   */
  async _onRelationshipAdd(data) {
    this.friendNicknames.set(data.id, data.nickname);
    this.cache.set(data.id, data.type);
    this.sinceCache.set(data.id, new Date(data.since || 0));

    if (this.client.options.autoAcceptFriends && data.type === RelationshipTypes.PENDING_INCOMING) {
      this.client.emit('debug', `[RELATIONSHIP] Auto-accepting friend request from ${data.id}`);
      await this.addFriend(data.id);
    }
  }

  /**
   * @param {UserResolvable} user Target user
   * @returns {Promise<Collection<Snowflake, User>>}
   */
  async fetchMutualFriends(user) {
    const id = this.resolveId(user);
    const data = await this.client.api.users(id).relationships['@mutual'].get();
    const mutuals = new Collection();
    for (const d of data) {
      const u = this.client.users._add(d);
      mutuals.set(u.id, u);
    }
    return mutuals;
  }
  ...
  /**
   * Clear pending friend requests.
   * @param {RelationshipType} [type] The type of relationship to clear (PENDING_INCOMING or PENDING_OUTGOING)
   * @returns {Promise<void>}
   */
  async clearPendingRequests(type) {
    const targets = this.cache.filter(t => t === type);
