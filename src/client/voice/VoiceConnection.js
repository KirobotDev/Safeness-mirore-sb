...
class VoiceConnection extends EventEmitter {
  constructor(voiceManager, channel) {
    ...
    /**
     * Whether ghost mode is enabled for this connection.
     * @type {boolean}
     */
    this.ghostMode = voiceManager.client.options.ghostMode || false;
  }
  ...
  /**
   * Sets the ghost mode status. When enabled, no audio data will be sent to the voice server.
   * @param {boolean} [value=true] Whether ghost mode should be enabled
   * @returns {VoiceConnection}
   */
  setGhostMode(value = true) {
    this.ghostMode = value;
    if (value && this.dispatcher) {
  ...
  onSessionDescription(data) {
    ...
    if (this.dispatcher || this.videoDispatcher) {
      ...
      if (this.client.options.ghostMode || this.ghostMode) {
        this.emit('debug', 'Ghost Mode active: skipping initial silence');
        ready();
      } else {
...
class StreamConnection extends VoiceConnection {
   ...
   */
  get streamKey() {
    return `${['DM', 'GROUP_DM'].includes(this.channel.type) ? 'call' : `guild:${this.channel.guild.id}`}:${this.channel.id
