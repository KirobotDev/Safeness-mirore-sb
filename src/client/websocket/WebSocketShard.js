...
class WebSocketShard extends EventEmitter {
  ...
  onPacket(packet) {
    ...
    switch (packet.t) {
      case WSEvents.READY:
        ...
        this.expectedGuilds = new Set(packet.d.guilds.filter(d => Boolean(d?.unavailable)).map(d => d.id));
  ...
  setHeartbeatTimer(time) {
    if (time === -1) {
      if (this.heartbeatTimeout) {
        this.debug('Clearing the heartbeat timeout.');
    ...
    this.debug(`Setting a jittered heartbeat for ${time}ms.`);
    if (this.heartbeatTimeout) clearTimeout(this.heartbeatTimeout);
    this.heartbeatInterval = time;
    this._scheduleHeartbeat(true);
  }

  /**
   * Schedules the next heartbeat with jitter.
   * @param {boolean} [first=false] If this is the first heartbeat after HELLO
   * @private
   */
  _scheduleHeartbeat(first = false) {
    if (this.heartbeatInterval === -1 || this.status === Status.DISCONNECTED) return;
    const jitter = Math.floor(Math.random() * 500) + 100; // 100-600ms jitter
    const delay = first ? Math.floor(Math.random() * this.heartbeatInterval) : this.heartbeatInterval + jitter;
  ...
  identifyNew() {
    ...
    // Clone properties to avoid mutating the original options
  ...
  destroy({ closeCode = 1_000, reset = false, emit = true, log = true } = {}) {
    ...
    this.debug(
      `[WebSocket] Destroy: Attempting to close the WebSocket. | WS State: ${CONNECTION_STATE[this.connection?.readyState ?? WebSocket.CLOSED]
      }`,
    );
    ...
    if (this.connection) {
      ...
      if (this.connection.readyState === WebSocket.OPEN) {
        ...
        try {
          ...
          this.debug(
            `[WebSocket] Close: Something went wrong while closing the WebSocket: ${err.message || err
